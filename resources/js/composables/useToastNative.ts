...
export const toast = (options: ToastOptions) => {
  // Toast function called with options
  ...
  try {
    ...
    if (!container) {
      ...
      // Container created and positioned
    }
    ...
    toast.style.cssText = `
      ...
      display: block;
    `
    // Toast element created with styles
    ...
    // Toast element added to DOM
    
    // Animate in
    setTimeout(() => {
      toast.style.opacity = '1'
      toast.style.transform = 'translateX(0)'
      // Toast animated into view
    }, 50)
    
    // Auto remove
    if (duration > 0) {
      setTimeout(() => {
        // Auto removing toast after duration
        removeToast(toastId)
      }, duration)
    }
    ...
    // Error creating toast - using fallback
    ...
    try {
      ...
      // Fallback toast created
    } catch (fallbackError) {
      // Fallback toast also failed
    }
  }
}
