...
const handleDrop = async (event: DragEvent, targetItemId: string) => {
    ...
    if (draggedItemData.parent_id === targetItemData.parent_id) {
        ...
        // Processing siblings at same level
        ...
        // Different parents - cross-level reordering not implemented
    }
        ...
        <div class="flex items-center justify-between">
            ...
            <Dialog v-model:open="isDialogOpen">
                ...
                <DialogContent class="sm:max-w-[90vw] md:max-w-[80vw] lg:max-w-[70vw] max-h-[95vh] overflow-y-auto">
                    ...
                    <div class="space-y-4 py-4">
                        <div class="flex items-center space-x-2">
                            <input 
                                ...
                                class="rounded border-input bg-background text-primary focus:ring-ring/20 dark:bg-background dark:border-input dark:text-primary disabled:cursor-not-allowed disabled:opacity-50"
                            />
                        ...
                        <template v-if="!formData.is_separator">
                            ...
                            <div class="space-y-2">
                                ...
                                <select id="parent" v-model="formData.parent_id" :disabled="formLoading" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-background dark:text-foreground dark:border-input">
                            ...
                            <div class="space-y-2">
                                ...
                                <Input id="position" v-model.number="formData.position" type="number" min="1" required :disabled="formLoading" />
                            </div>
                            
                            <div class="space-y-2">
                                <Label for="badge">Badge (optional)</Label>
                                <Input id="badge" v-model="formData.badge" placeholder="New" :disabled="formLoading" />
                            </div>
                    ...
                    <DialogFooter>
                        <Button type="button" variant="outline" @click="isDialogOpen = false" :disabled="formLoading">
                            Cancel
                        </Button>
                        <Button type="button" @click="saveMenuItem" :disabled="formLoading">
        ...
        <Card>
            ...
            <CardContent>
                ...
                <div v-else-if="error" class="text-destructive py-4">
                    Error: {{ error }}
                    <Button @click="fetchMenuItems" variant="outline" size="sm" class="ml-2" :disabled="loading">
                ...
                <div v-else class="space-y-2 relative">
                    ...
                    <div v-if="reorderLoading" class="absolute inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-10">
                        <div class="flex items-center gap-2 bg-background px-4 py-2 rounded-lg border shadow-lg">
                            <Loader2 class="w-4 h-4 animate-spin" />
                            <span class="text-sm">Updating menu order...</span>
                        </div>
                    </div>
