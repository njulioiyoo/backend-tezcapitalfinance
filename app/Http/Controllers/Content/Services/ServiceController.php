...
class ServiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'interest_list.*.name' => 'required|string|max:255',
            'interest_list.*.rate' => 'required|numeric|min:0|max:100',
            'document_list' => 'nullable|array', 
            'document_list.*' => 'string',
            'fees_list' => 'nullable|array',
            'fees_list.*.name' => 'required|string|max:255',
            'fees_list.*.type' => 'required|in:percentage,fixed',
            'fees_list.*.value' => 'required|numeric|min:0',
        ...
        try {
            ...
            $data = [
                ...
                'interest_list' => $request->interest_list && is_array($request->interest_list) && !empty(array_filter($request->interest_list)) ? $request->interest_list : null,
                'document_list' => $request->document_list && is_array($request->document_list) && !empty(array_filter($request->document_list)) ? $request->document_list : null,
                'fees_list' => $request->fees_list && is_array($request->fees_list) && !empty(array_filter($request->fees_list)) ? $request->fees_list : null,
